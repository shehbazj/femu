...
int main(int argc, char *argv[])
{
	...
	if(pid == 0) {	
		...
		if (!is_fdopen(send_to_compression_fd)) {
			printf("open compression fd send pipe\n");
			send_to_compression_fd = open("compression_pipe_send", O_WRONLY, 0666);
		...
		if (!is_fdopen(recieve_from_compression_fd)) {
			printf("open compression fd recv pipe\n");
			recieve_from_compression_fd = open("compression_pipe_recv", O_RDONLY, 0666);
	...
	close(recieve_from_compression_fd);
