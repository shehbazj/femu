...
local void treat_file   (int in_fd, int of_fd);
...
int gzip_me(int in_fd, int out_fd, int mode);

/* ======================================================================== */
//int main (int argc, char **argv)
//int gzip_me(char *infile, char *outfile, int mode)
int gzip_me(int in_fd, int of_fd, int mode)
{
...
int main(int argc, char *argv[])
{
	...
	if (in_fd < 0) {
		printf("error opening readfd\n");
		exit(0);
	}

	int out_fd = open(argv[2], O_WRONLY);
	if (out_fd < 0) {
		printf("error opening writefd\n");
		exit(0);
	}
	gzip_me(in_fd, out_fd, option);
}
*/
...
//    char *iname;
local void treat_file(if_fd, of_fd)
	int if_fd;
	int of_fd;
{
...
local void copy_stat(ifstat)
    struct stat *ifstat;
{
    mode_t mode = ifstat->st_mode & S_IRWXU;
    int r;
